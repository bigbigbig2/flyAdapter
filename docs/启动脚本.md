...
Aurora Agent 必须运行在 Aurora SDK 依赖完整的环境里。不要在主 Adapter 里 import Aurora SDK，也不要让主 Adapter 反复 `docker exec`。

如果 Aurora SDK 只能在 `fourier_aurora_server` 容器里 import，就在容器里进入 `gr3` 目录后启动本 Agent；如果宿主机有同等 SDK 环境，也可以在宿主机启动。
...
如果 `cd ~/aurora_ws/flyAdapter` 报 `No such file or directory`，不要继续执行后面的命令。你贴的那次日志里，脚本实际从 `~/aurora_ws/flyAdapter` 启动，所以 Agent 打印的 root/sys.path 也是 `flyAdapter`。现场要么把当前工程部署到 `~/aurora_ws/flyAdapter`，要么把文档里的目录改成真实目录，但不要在 `cd` 失败后继续启动。
...
- `/robot/status` 里 `aurora.backend=agent`，说明 Adapter 正在使用 Aurora Agent。
...
如果 `/robot/aurora/state` 不通，先检查 Aurora Agent，而不是检查主 Adapter 的 Python 环境：
...
如果 Agent 的 `/health` 里仍然提示缺少 `fourier_msgs.msg.AuroraCmd`，说明 Agent 所在环境仍不是正确的 Aurora SDK 环境，需要把 Agent 放到 SDK 依赖完整的容器/环境里运行。
现在 `run_aurora_agent.sh` 会默认清理继承来的 HumanoidNav overlay，避免加载导航版 `fourier_msgs`。如果仍然缺 `AuroraCmd`，看 `/health` 返回里的 `module_diagnostics`，其中会显示当前 `fourier_msgs` 实际来自哪个路径。
...
Unmatched subscriber: rt/robot_stand_pose_state
```

结论不是 Python 包缺失，而是 Aurora SDK 已经启动 DDS，但没有匹配到 AuroraCore 发布者。按这个顺序查：

```bash
# 1. 在运行 AuroraCore 的容器/环境里确认配置
grep -E "DomainID|RobotName|RunType" /workspace/config/config.yaml

# 2. 确认和 Agent 一致
echo "$AURORA_DOMAIN_ID $AURORA_ROBOT_NAME"

# 3. 如果 AuroraCore 在 fourier_aurora_server 容器里，优先把 Agent 也放进同一个容器运行
sudo docker exec -it fourier_aurora_server bash
cd /workspace/gr3 || exit 1
./scripts/run_aurora_agent.sh
```

修正 AuroraCore、DomainID、RobotName 或容器网络后，可以让 Adapter 调 Agent 重置 SDK 客户端：
...
curl -X POST http://127.0.0.1:8080/robot/map/load \
  -H "Content-Type: application/json" \
  -d '{"map_path":"/home/gr301ab0113/aurora_ws/flyAdapter/data/maps/map","x":0,"y":0,"yaw":0}'
```
