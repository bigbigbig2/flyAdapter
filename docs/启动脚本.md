...
如果 `cd ~/aurora_ws/gr3` 报 `No such file or directory`，不要继续执行后面的命令。你贴的那次日志里，脚本实际从 `~/aurora_ws/flyAdapter` 启动，所以 Agent 打印的 root/sys.path 也是 `flyAdapter`。现场要么把当前工程部署到 `~/aurora_ws/gr3`，要么把文档里的目录改成真实目录，但不要在 `cd` 失败后继续启动。
