...
如果 `/robot/aurora/state` 不通，先确认当前 adapter 使用的 Python 环境能直接 import Aurora SDK：
...
如果 Aurora SDK 只能在 `fourier_aurora_server` 容器里 import，就要在容器/同等 SDK 环境里启动 adapter，而不是让宿主机上的 adapter 反复跨容器调用。
